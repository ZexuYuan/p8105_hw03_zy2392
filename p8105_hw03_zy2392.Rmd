...
```{r, fig.width=2000, fig.height=10}
filter_aisle = 
  instacart %>% 
  group_by(aisle) %>% 
  summarise(order_id = n()) %>% 
...
ggplot(filter_aisle, aes(x = aisle, y = order_id)) + geom_point() + labs(title = "Number of orders for each aisle", x = "Aisle", y = "Number of orders")
```
...
three_most_pop_items = 
  ...
  summarise(product_num = n(), .group = 'keep') %>% 
  mutate(product_num_rank = rank(desc(product_num))) %>% 
  filter(product_num_rank <= 3) %>% 
  select(-product_num_rank) %>% 
  knitr::kable()
```
...
The mean order hour in a week for Coffee Ice Cream are from 13:00 to 15:00 approximately; and the mean order hour in a week for Pink Lady Apple are around from 8:00 to 16:00.
...
The 'spaghetti' plot is messy, so it is least possible to obtain useful information of the dataset by visualization.
...
Final dataset should include all originally observed variables and values; have useful variable names; include a weekday vs weekend variable; encode data with reasonable variable classes. 
...
accel_agg = 
  accel %>% 
  mutate(total_act = rowSums(across(activity_1:activity_1440))) %>% 
  relocate(week_id, day_id, week, day, total_act)

accel_agg_plot = 
  accel_agg %>% 
  group_by(day) %>% 
  summarise(total_act_mean = mean(total_act)) %>% 
  mutate(day = factor(day, levels = c('Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday','Friday', 'Saturday')))

ggplot(accel_agg_plot, aes(x = day, y = total_act_mean, color = day)) + geom_bar(stat = "identity")
```

The patient has the highest activity value on average on Friday whereas the lowest value on Saturday.
...
The patient is active around the afternoon on Saturday and Sunday, and active around 20:00 in the night on Friday; he or she is basically not active between 0:00 am to 6:00 am, it is probably his or her sleep time.
